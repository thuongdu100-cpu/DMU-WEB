...
function compactWeekRangeLabel(label) {
  const s = String(label || "").trim();
  const m = /(\d{1,2})\/(\d{1,2})\s*[–-]\s*(\d{1,2})\/(\d{1,2})(?:\/\d{2,4})?/.exec(s);
  if (!m) return s;
  return `${Number(m[1])}/${Number(m[2])}-${Number(m[3])}/${Number(m[4])}`;
}

/** Đường cong Catmull–Rom (qua cubic Bézier) qua các điểm */
function smoothLineThroughPoints(points, tension = 1.35, minY = Number.NEGATIVE_INFINITY, maxY = Number.POSITIVE_INFINITY) {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const cp1y = clampY(p1.y + ((p2.y - p0.y) / 6) * t);
    const cp2x = p2.x - ((p3.x - p1.x) / 6) * t;
    const cp2y = clampY(p2.y - ((p3.y - p1.y) / 6) * t);
...
function rangeCaption(series) {
  ...
  if (series.length === 1) {
    const single = compactWeekRangeLabel(series[0].label);
...
function WeekWaveChartSvg({ series, title, stroke, fill }) {
  ...
  const model = useMemo(() => {
    ...
    const linePath = points.length ? smoothLineThroughPoints(points, 1.35, pad.t, baseY) : "";
    let areaPath = "";
    if (points.length >= 2) {
...
export function AdminDashboard() {
  ...
  const isDaily = data.periodType === "day";
  ...
  return (
    <>
      ...
      <p className="admin-lead">
        {isDaily ? (
          <>
            Mỗi lần hiển thị <strong>7 ngày realtime</strong> (khung đầu tiên gồm hôm nay). Dùng nút để xem{" "}
            <strong>7 ngày trước đó</strong> hoặc quay lại gần hiện tại.
          </>
        ) : (
          <>
            Mỗi lần hiển thị <strong>1 tuần</strong> (khung đầu tiên là tuần hiện tại). Dùng nút để xem{" "}
            <strong>tuần trước đó</strong> hoặc quay lại gần hiện tại.
          </>
        )}
      </p>
      ...
      <div className="admin-chart-nav" role="group" aria-label="Chọn khung thời gian">
        <button type="button" className="btn-admin" disabled={block >= MAX_BLOCK || loading} onClick={goOlder}>
          {isDaily ? "← 7 ngày trước" : "← Tuần trước"}
        </button>
      ...
      <div className="admin-card admin-chart-card">
        ...
        <WeekWaveChartSvg
          ...
          title={isDaily ? "Lượt truy cập theo ngày" : "Lượt truy cập theo tuần"}
      ...
      <div className="admin-card admin-chart-card">
        ...
        <p className="admin-chart-sub">
          {isDaily ? "Theo ngày xuất bản/cập nhật gần nhất" : "Theo ngày tạo bài (đã đăng)"} · {rangeArticles}
        </p>
        <WeekWaveChartSvg
          series={data.articlesPublishedByWeek}
          title={isDaily ? "Số bài đăng theo ngày" : "Số bài đăng theo tuần"}
